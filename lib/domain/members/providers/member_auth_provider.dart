    ...
    try {
      ...
      if (sessionUser != null && token != null && token.isNotEmpty) {
        ...
        if (token == null || token.isEmpty)
          errorDetail = "토큰 누락";
        else if (sessionUser == null) errorDetail = "사용자 정보 누락";
        throw Exception("로그인 처리 중 알 수 없는 문제가 발생했습니다 ($errorDetail).");
      }
    } catch (e) {
      final errorMessage = extractErrorMessage(e);
      state =
  ...
  Future<bool> refreshAccessToken() async {
    try {
      ...
      if (newAccessToken != null && newAccessToken.isNotEmpty) {
        ...
        print(
            "Failed to refresh access token (AuthNotifier): New access token is null or empty.");
        await logout();
        return false;
      }
    } catch (e) {
