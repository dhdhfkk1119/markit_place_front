...
import '../../../_core/utils/my_http.dart'; // dio 인스턴스 및 secureStorage 인스턴스 사용
import '../models/session_user.dart';
import '../dtos/login_response.dto.dart';

// Secure Storage 키 정의 (SessionRepository에서 가져옴)
...
class MemberAuthRepository {
  final Dio _dio = dio; // my_http.dart의 dio 인스턴스 사용
  // final FlutterSecureStorage _secureStorage = const FlutterSecureStorage(); // secureStorage를 직접 초기화하거나 my_http.dart의 인스턴스 사용
  final FlutterSecureStorage _secureStorage =
      secureStorage; // my_http.dart에 정의된 전역 인스턴스 사용

  MemberAuthRepository(); // 생성자에서 FlutterSecureStorage를 주입받지 않고 직접 사용 또는 전역 인스턴스 사용

  /// API: 계정 기반 로그인 처리 (기존 MemberAuthRepository의 login 메소드와 동일)
  Future<Map<String, dynamic>> login(String loginId, String password) async {
    try {
      ...
      print(
          '[MemberAuthRepository Login Error - General] \$errorMessage (\$e)');
      throw Exception(errorMessage);
    }
  }

  // --- SecureStorage 관련 메소드 (SessionRepository에서 가져옴) ---
  ...
  Future<void> storeSessionData(SessionUser sessionUser, String token) async {
    ...
    print("[MemberAuthRepository] Session data stored in SecureStorage.");
  }
  ...
  Future<SessionUser?> getStoredUser() async {
    ...
    if (memberIdStr != null && role != null) {
      try {
        ...
        print("[MemberAuthRepository] Error parsing stored user data: \$e");
  ...
  Future<void> clearSessionData() async {
    ...
    print("[MemberAuthRepository] Session data cleared from SecureStorage.");
  }
  ...
  // --- 에러 핸들링 (기존 _handleDioError 메소드와 거의 동일, 로그 Prefix만 다를 수 있음) ---
